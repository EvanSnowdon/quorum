...
0. **Lock the scope before anything else.** Open the plan with a **Scope Lock** block that every downstream analyst must treat as binding. It fixes: the **market definition and denominator basis** (exactly what is counted in the market — units, revenue, installed base — and what the denominator of any share or penetration figure is); the **reference year** every figure anchors to (data from other years must be flagged as such); the **currency** (and nominal vs. real); the **geographic boundary**; and an explicit list of **adjacent categories excluded, with the reason for each exclusion**. Where the industry has overlapping category framings, the Scope Lock must adjudicate them explicitly rather than leave the choice implicit — e.g., for electric vehicles it must rule BEV-only versus NEV (BEV + PHEV) and say which one all sizing and share figures use. If the market definition locks a single powertrain or technology form, the Scope Lock must also state that substitution from the adjacent forms is covered by the competition and demand sections — the sizing denominator is locked, the competing set is not. The denominator choice (e.g. NEV vs BEV-only) must state its decision criterion (what question the engagement answers), and when engagement memory shows a prior engagement on the same market used a different denominator, the Scope Lock must acknowledge and justify the change rather than silently flip. A plan without a Scope Lock is not a plan; a section that drifts from the Scope Lock is a defect.
...
- **Open with the Scope Lock.** The very first block of the plan is a section headed `Scope Lock` containing, as labelled lines: market definition and denominator basis; reference year; currency; geographic boundary; excluded adjacent categories with reasons (the explicit adjudication from Method step 0). Every downstream section inherits these settings; any figure that departs from them (a different year, a wider category) must say so inline.
